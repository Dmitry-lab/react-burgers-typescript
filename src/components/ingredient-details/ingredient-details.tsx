...
function IngredientDetails() {
  ...
  const { viewedIngredient, ingredients } = useSelector((store: RootState )=> store.burgersConstructor);
  const { id: ingredientId } = useParams<{id: string}>();

  const info = viewedIngredient ? viewedIngredient : ingredients.find(item => item._id === ingredientId)
