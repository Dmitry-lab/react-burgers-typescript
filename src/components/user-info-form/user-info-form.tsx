...
function UserInfoForm() {
  ...
  const { info, requestError } = useSelector((store: RootState) => store.userInfo);
  ...
  const nameChangeHandler = (e: FormEvent<HTMLInputElement>) => {
    ...
    (e.currentTarget.value !== info.name) ? setNameChanged(true) : setNameChanged(false);
  }
  ...
  const loginChangeHandler = (e: FormEvent<HTMLInputElement>) => {
    setLogin(e.currentTarget.value);
    (e.currentTarget.value !== info.email) ? setLoginChanged(true) : setLoginChanged(false);
  }

  const passwordChangeHandler = (e: FormEvent<HTMLInputElement>) => {
    setPassword(e.currentTarget.value);
    (e.currentTarget.value !== '') ? setPasswordChanged(true) : setPasswordChanged(false);
  }
