...
const OrderItem: FC<TOrederProps> = ({ order, onClick }) => {
  ...
  const ingredientsInfo = useMemo(() => {
    return order.ingredients.map((orderIngredient: string) => {
      return ingredientsList?.find((ingredient: TIngredient) => ingredient._id === orderIngredient)
    })
