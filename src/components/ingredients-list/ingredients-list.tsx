...
function IngredientsList() {
  ...
  const { ingredients, currentTab, scrollTarget } = useSelector((store: RootState) => store.burgersConstructor)
  ...
  const specificList = (ingredients: Array<TIngredient>, type: string) => {
  ...
  const switchTabCheck = () => {
    ...
    if (listPosition && bunsRef.current && saucesRef.current && mainRef.current) {
  ...
  const handlerOpenModal = (info: TIngredient) => {
    setModalOpened(true);
  ...
  useEffect(() => {
    ...
    if (bunsRef.current && saucesRef.current && mainRef.current && listsRef.current) { 
